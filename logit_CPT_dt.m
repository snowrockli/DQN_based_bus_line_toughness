function cell=logit_CPT_dt(dt,i,j,cell,OD,theita,shutdown)
%==================先计算效用===================
...
cell{i,j}.bus_cpt_utility(:,dt)=f_u(1:cell{i,j}.route_num);
cell{i,j}.metro_cpt_utility(dt)=f_u(cell{i,j}.route_num+1);
%=======================分流===================================
if i~=j
    if ismember(i,shutdown)==1||ismember(j,shutdown)==1%出现停运的状况
        ...
        %========选择公交出行人数===========
        for k=1:cell{i,j}.route_num
            cell{i,j}.bus_q(k,dt)=OD(i,j)*cell{i,j}.wd(dt)*exp(theita*cell{i,j}.bus_cpt_utility(k,dt))/sum(exp(theita*cell{i,j}.bus_cpt_utility(:,dt)));
        end
    else
        %========选择地铁出行人数=========
        cell{i,j}.metro_q(dt)=OD(i,j)*cell{i,j}.wd(dt)*exp(theita*cell{i,j}.metro_cpt_utility(dt))/(sum(exp(theita*cell{i,j}.bus_cpt_utility(:,dt)))+exp(theita*cell{i,j}.metro_cpt_utility(dt)));
        %========选择公交出行人数===========
        for k=1:cell{i,j}.route_num
            cell{i,j}.bus_q(k,dt)=OD(i,j)*cell{i,j}.wd(dt)*exp(theita*cell{i,j}.bus_cpt_utility(k,dt))/(sum(exp(theita*cell{i,j}.bus_cpt_utility(:,dt)))+exp(theita*cell{i,j}.metro_cpt_utility(dt)));
        end
    end
end
end